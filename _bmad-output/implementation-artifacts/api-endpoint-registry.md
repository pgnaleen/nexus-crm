...
| 5 | DELETE | `/employees/:id` | RBAC | `EMPLOYEES_DELETE` | Soft-delete an employee record created by mistake (Story 1.5) — deliberately distinct from "Mark as Exited" (a PATCH keeping the record visible for compliance/history). | none | `{success: true}` | `remove` → `employees.service.ts::remove` | `EmployeesWidget.tsx` (row trash icon → `useConfirm` destructive dialog, same pattern as Departments) | ✅ | **New 2026-07-23**. `EMPLOYEES_DELETE` created by this story (seeded via `Object.values(PERMISSIONS)` — re-run seed). Same two-step `softRemove()`+`update(deletedBy)` repository pattern as Teams/Departments; bare-loaded mutation target. Records an `audit_logs` delete entry (`{fullName, employeeCode}` snapshot). Uploaded photo/CV files are deliberately NOT deleted (record is recoverable at the DB level). |
...
| 5 | PATCH | `.../parties/companies/:mapId` | RBAC | `RELATIONSHIP_UPDATE` | Update a tagged Company's fields. | body: partial company fields | `RelationshipPartyResponse` | `updateCompany` → `relationship-parties.service.ts::updateCompany` | `CompanyFormDialog.tsx` (edit) | ✅ | Records an `audit_logs` update diff (`entityType: "company"`). **2026-07-27**: `company.logo` now stores an S3 key (see Uploads section below) instead of a local disk path; a replaced logo's old S3 object is now deleted (best-effort, never fails the update) — was previously never cleaned up at all. `CompanyResponse.company.logoDisplayUrl` (a fresh signed URL, generated per response) added for rendering — `logo` itself is submit-back-safe, never a URL. |
...
`_bmad-output/implementation-artifacts/spec-relationship-tags-tab.md`). Cross-relationship-type tag list/add for a
...
| 3 | POST | `/users` | RBAC | `USERS_CREATE` | Create a user (optionally assigning roles and linking an Employee HR record). | body: `{username, displayName, loggingEmail, password, status?, mustChangePassword?, extras?, roleIds?, employeeId?}` | `UserResponse` | `create` → `users.service.ts::create` | Users admin widget (Add) | ✅ | Now records an `audit_logs` insert — deliberately excludes the password; only `username`/`displayName`/`status`. **2026-07-23 (Story 1.6):** optional `employeeId` links the new account to an employee via `EmployeesService.linkToUser` — 409 if that employee is already linked to another account; the employee-side change gets its own `audit_logs` update (`entityType: "employee"`, `userId` diff). |
