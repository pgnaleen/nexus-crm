...
| 7 | GET | `/pickers/deal-customer-parties` | System-Internal (Picker) | `DEALS_VIEW` | Companies/contacts tagged under whichever Relationship Type this tenant has flagged as its `systemRole: CUSTOMER` type, for Add Deal's Customer field. Resolves the flagged type by id (never by name), so renaming it never breaks the filter. Company-owned contacts inherit their parent company's tag. | none | `RelationshipRolePickerResponse` → `{configured: boolean, companies: CompanyPickerResponse[], contacts: ContactPickerResponse[]}` | `pickers.controller.ts::findDealCustomerParties` → `relationship-types.service.ts::findSystemRoleTypeId` + `companies.service.ts::findPickerForRelationshipType` + `contacts.service.ts::findPickerForRelationshipType` | `lib/api/pickers.ts` + `lib/pickers/server.ts` — AddDealDialog's Customer field | ✅ | New — F2 fix (`todo-master-remaining-work.md`). `configured: false` means no type is flagged yet; distinct from `configured: true` with empty arrays, which means a type is flagged but nothing's tagged under it. |
| 8 | GET | `/pickers/deal-partner-parties` | System-Internal (Picker) | `DEALS_VIEW` | Same as #7, for the tenant's `systemRole: PARTNER` type, feeding Add Deal's Partners field. | none | `RelationshipRolePickerResponse` (same shape as #7) | `pickers.controller.ts::findDealPartnerParties` → same service methods as #7, `SystemRole.Partner` | `lib/api/pickers.ts` + `lib/pickers/server.ts` — AddDealDialog's Partners field | ✅ | New — F2 fix, paired with #7. |
