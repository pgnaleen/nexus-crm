...
- **Debug Logging** — ✅ endpoint + its service method(s) follow the "Deep debug logging inside
  ...
  `../6-finished-archive/todo-audit-infrastructure.md`. Scan this column to see what's left.
...
gate did) — see the consolidation writeup in `../6-finished-archive/todo-audit-infrastructure.md` for the full history,
...
**2026-07-27**: `profilePhotoUrl`/`cvUrl` (wherever present below) are now S3 keys (see Uploads
section); responses that include them also carry fresh `profilePhotoDisplayUrl`/`cvDisplayUrl`
(signed, generated per response) for rendering.
...
**2026-07-27**: `evidenceFileUrl` on every response below is now an S3 key (see Uploads section);
responses that include it also carry a fresh `evidenceFileDisplayUrl` (signed, generated per
response) for rendering — `evidenceFileUrl` itself is submit-back-safe, never a URL.
...
| 5 | PATCH | `.../parties/companies/:mapId` | RBAC | `RELATIONSHIP_UPDATE` | Update a tagged Company's fields. | body: partial company fields | `RelationshipPartyResponse` | `updateCompany` → `relationship-parties.service.ts::updateCompany` | `CompanyFormDialog.tsx` (edit) | ✅ | Records an `audit_logs` update diff (`entityType: "company"`). **2026-07-27**: `company.logo` now stores an S3 key (see Uploads section below) instead of a local disk path; a replaced logo's old S3 object is now deleted (best-effort, never fails the update) — was previously never cleaned up at all. `CompanyResponse.company.logoDisplayUrl` (a fresh signed URL, generated per response) added for rendering — `logo` itself is submit-back-safe, never a URL.  **2026-07-31 — multi-country/multi-industry.** `country: string` → `countries: string[]` (jsonb on `companies`, alongside the existing `brands`/`branches` arrays) and `industryId: uuid` → `industryIds: uuid[]` (a `company_industries` join table; the old FK column is dropped). `CompanyResponse` also gains `industryNames` so lists render without a second lookup, mirroring `territoryOwnerName`. Migration `1784700000030` backfills both from the old columns and aborts if any row fails to migrate. On PATCH, an omitted `industryIds` leaves the links untouched while `[]` clears them; the links are diffed and audited explicitly (`changes.industryIds`), since the generic `Object.keys(companyDto)` diff loop cannot see a field that is not a column. `industryIds` is destructured off the DTO before `Object.assign`, deliberately — leaving it in would put a non-column field through the bare-load-mutate-save path CLAUDE.md's TypeORM Gotcha section documents. |
...
**New 2026-07-30**, added with the Relationships tab feature (spec:
`_bmad-output/3-feature-specs/spec-relationship-tags-tab.md`). Cross-relationship-type tag list/add for a
...
New module, 2026-08-03 — built against `spec-activity-log.md` (System Administration page),
mock-first per `feature-development-guideline.md`: the frontend (`ActivityLogWidget.tsx`, under
the "Settings" sidebar entry's "Audits" tab, alongside the pre-existing Backups tab — see that
page's own comment for why these two share one nav entry instead of each getting its own) was
built and signed off on local mock data before this backend existed, then wired to the real
endpoints below in the same pass this table row was added.
...
`AuthService.login()`/`AuthService.logout()` (see the Auth module section above) at every branch —
unknown username, inactive account, still-locked-out, wrong password, the exact-moment lockout is
newly triggered, successful login, and logout. `AuthEventService.record()` is best-effort, same
posture as `AuditLogService.record()`: a failed write is logged and swallowed, never fails the real
login/logout it's attached to.
...
`spec-activity-log.md`'s own v1 decision (which deferred it entirely) — added per an explicit,
separate product decision: every non-System tenant can only ever see its own activity; the System
tenant can see everything by default, or narrow to one tenant via the Tenant filter. Verified live
end to end: a real System-tenant Super Admin session got 220 rows with `allTenants=true` (218
System + 2 Helix) and exactly 2 rows with `tenantId=<Helix>`; a real Helix (non-platform) test
user holding only `AUDIT_LOG_VIEW` got the identical 2-row Helix-only result regardless of
...
**Deliberately still dummy** (no query, no product decision made yet — see CLAUDE.md-adjacent
plan notes): `TargetRevenueGaugeWidget` (no quota/target table anywhere in the schema),
the Pipeline Coverage stat card (same missing denominator), `WinLossReasonsChartWidget` (no
win/loss reason field on Deal), `TeamPerformanceRadarWidget` (only "deals closed" is derivable —
response time/follow-ups/upsells/satisfaction have no backing columns anywhere).
