...
| 2 | POST | `/deals/:dealId/documents` | RBAC | `DEALS_UPDATE` | Upload a document to S3 (memory-buffered, per-deal key prefix), metadata row created. | multipart: `file`, `docType`, `title` | `DealDocumentResponse` | `create` → `deal-documents.service.ts::create` | `AddDealDialog.tsx` (both modes) | ✅ | Records an `audit_logs` insert (`entityType: "deal_document"`). **2026-07-27**: switched from local disk (`multer.diskStorage`) to S3 (`multer.memoryStorage` → `S3Service.putObject`), via the shared `core/storage/s3.service.ts` (same bucket as DB backups, `uploads/` key prefix). |
