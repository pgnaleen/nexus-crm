...
- source_spec: `_bmad-output/3-feature-specs/spec-relationship-tags-tab.md`
  ...
  evidence: "`CompanyFormDialog.tsx`/`ContactFormDialog.tsx`'s `relationshipTypeOptions` effect runs once per `[mode, canCreate]` (or `showRelationshipsTab`); `handleAddTag`'s success path only calls `refreshTags()` (the party's own tag list), never re-fetches the picker options. Low-probability window (another admin editing Relationship Types during the exact time this dialog is open) with a low-severity outcome (stale dropdown, not incorrect data). **Correction, 2026-07-31:** 'not incorrect data' understated it. The same staleness pattern on the Company form's new Industries picker was the realistic trigger for a silent data-loss bug (a stale id reaching a non-transactional delete-then-insert wiped every industry link on the company) -- see the 'Found and fixed' section of open-items.md. That specific destructive path is fixed, so this entry stays deferred at its original priority, but the general lesson holds: a stale picker option is not harmless on its own, it is harmless only if every write path it feeds validates the id and is atomic."
