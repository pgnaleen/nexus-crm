...
export default async function LeadsPage({ params }: { params: { tenant: string } }) {
  const session = await getServerSession(params.tenant);
  if (!session) {
    redirect(`/${params.tenant}`);
  }

  return <ComingSoonSection title={t("leads.title")} subtitle={t("leads.subtitle")} icon={<LeadsIcon size={22} />} />;
}
